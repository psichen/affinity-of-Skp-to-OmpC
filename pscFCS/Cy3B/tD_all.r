...
for(rep in reps){
    ...
    p.fcs[[rep]] <- ggplot(fcs)+
        geom_line(aes(time, g))+
        geom_line(aes(time, predict), color='red')+
        facet_wrap(~pt)+
        scale_x_log10(name='Lag time / s', breaks=10^(-6:0), limits=c(1e-6, 1), minor_breaks=as.numeric(1:10%o%10^(-6:0)), labels = trans_format("log10", math_format(10^.x)))+
        scale_y_continuous(name='Correlation', limits=c(1, 5))+
        theme_bw()
}

p <- ggplot(fcs.coef)+
    geom_point(aes(pt, tD*1e6, col=factor(rep)))+
