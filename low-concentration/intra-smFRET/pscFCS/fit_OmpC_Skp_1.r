...
p.fcs.all <- ggplot(fcs.all)+geom_line(aes(time, g, color=factor(pt)))+scale_x_log10()+facet_wrap(~factor(rep))+labs(x='lag time / s', y='correlation')
p.tD.all <- ggplot(fcs.coef)+geom_point(aes(pt, tD*1e6, col=factor(rep)))+geom_line(aes(pt, fit*1e6, col=factor(rep)))+facet_wrap(~sam)+labs(x='peak threshold', y=expression('diffusion time /'~mu*s))
p.G0.all <- ggplot(fcs.coef)+geom_point(aes(pt, G0, col=factor(rep)))+facet_wrap(~sam)+labs(x='peak threshold', y='G0')

if(p.tD.all.save){
    ggsave(paste0('./',sam,'_',interval,'.png'), p.tD.all)
}

if(tD.csv.write){
    write.csv(tD.csv, paste0('./tD_',sam,'_',interval,'.csv'), row.names=F)
}
